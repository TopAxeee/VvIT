...
object RecursiveFunctions {
  ...
  def reverse[A](list: List[A]): List[A] = {
    ...
    def loop(origin: List[A], reverse: List[A]): List[A] = origin match {
      case Cons(n, t) => loop(t, Cons(n, reverse))// -голова reverse
      case Nil() => reverse
    }
    loop(list, new Nil[A])
  }
  ...
  def map[A,B](list: List[A])(f: A => B): List[B] = {
    ...
    def loop(origin: List[A], changed: List[B]): List[B] = origin match {
      case Cons(n, t) => loop(t, Cons(f(n), changed))// -голова f
  ...
  def append[A](l: List[A], r: List[A]): List[A] = {
    @tailrec
    def loop(left: List[A], right: List[A] ): List[A] = left match { //reverse -голова
  ...
  def flatMap[A, B](list: List[A])(f: A => B): List[B] = {
    ...
    def loop(origin: List[A], result: List[B]): List[B] = origin match {
      case Cons(n, t) => loop(t, Cons(f(n), result))// -голова f reverse
  ...
  /** Зависит от задания. Например если мы хотим найти какое-то значение в бинарном дереве поиска, то алгоритм можно
  ...
  def main(args: Array[String]): Unit = {
    println("testReverse(Cons(1, Cons(2, Cons(3, Nil())))):" + testReverse(Cons(1, Cons(2, Cons(3, Nil())))))
    println("testMap(Cons(1, Cons(2, Cons(3, Nil()))), (x: Int) => x + 2):" + testMap(Cons(1, Cons(2, Cons(3, Nil()))), (x: Int) => x + 2))
    println("testAppend(Cons[Int](1, Cons(2, Cons(3, Nil()))), Cons[Int](4, Cons(5, Cons(6, Nil())))):" + testAppend(Cons[Int](1, Cons(2, Cons(3, Nil()))), Cons[Int](4, Cons(5, Cons(6, Nil())))))
    println("flatMap(Cons[Int](1, Cons(2, Cons(3, Nil()))))((x: Int) => Cons(x, Nil())):" + flatMap(Cons[Int](1, Cons(2, Cons(3, Nil()))))((x: Int) => Cons(x, Nil())))
  }
}
